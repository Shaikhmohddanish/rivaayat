                ...
                <Card key={order._id}>
                  ...
                  <CardContent className="p-3 sm:p-6 pt-0">
                    <div className="space-y-3 sm:space-y-4">
                      <div>
                        <h4 className="font-semibold mb-2 text-sm sm:text-base">Customer</h4>
                        <div className="text-xs sm:text-sm space-y-1">
                          <p className="font-medium">
                            {order.user?.name || order.shippingAddress?.fullName || "Unknown customer"}
                          </p>
                          <p className="text-muted-foreground">
                            {order.user?.email || order.shippingAddress?.email || "Email not available"}
                          </p>
                          {order.userId && (
                            <Link
                              href={`/admin/users?userId=${order.userId}`}
